...
build_farmos_dev () {
  ...
  drush make --working-copy --no-gitinfofile /var/farmOS/build-farm.make.yml /tmp/farmOS \
...
rebuild_required () {
  ...
  if ! [ -e /var/www/html/profiles/farm/farm.info.yml ]; then
    echo >&2 "farmOS not detected. Building..."
    return 0
  fi

  # Get the current version of farmOS from the installation profile info file.
  CURRENT_VERSION="$(grep 'version' profiles/farm/farm.info.yml | sed 's/version: \(.*\)/\1/')"
  ...
  echo >&2 "To force a rebuild, delete profiles/farm/farm.info.yml and restart the container."
  return 1
}
